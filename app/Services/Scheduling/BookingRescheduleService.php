...
class BookingRescheduleService
{
    ...
    public function reschedule(
        ...
        if ($reason === '') {
            throw ValidationException::withMessages(['reason' => 'Alasan reschedule wajib dicatat.']);
        }

        if ($capacityOverride && ! in_array($actor->role, [Role::SUPER_ADMIN, Role::ADMIN], true)) {
            throw ValidationException::withMessages(['capacity_override' => 'Capacity override hanya boleh dilakukan oleh super admin atau admin.']);
        }

        if ($capacityOverride && $capacityOverrideReason === '') {
            throw ValidationException::withMessages(['capacity_override_reason' => 'Alasan capacity override wajib dicatat.']);
        }
        ...
        return DB::transaction(function () use (
            ...
            $source = ChildSessionBooking::query()->whereKey($sourceBooking->id)->lockForUpdate()->firstOrFail();
            $destination = SessionOccurrence::query()->whereKey($destinationOccurrence->id)->lockForUpdate()->firstOrFail();

            if ($source->status !== 'scheduled') {
                throw ValidationException::withMessages(['source_booking_id' => 'Hanya booking aktif berstatus scheduled yang dapat dipindahkan.']);
            }

            if ((int) $source->session_occurrence_id === (int) $destination->id) {
                throw ValidationException::withMessages(['destination_occurrence_id' => 'Occurrence tujuan harus berbeda dari occurrence sumber.']);
            }

            if (in_array($destination->status, ['cancelled', 'completed'], true)) {
                throw ValidationException::withMessages(['destination_occurrence_id' => 'Occurrence tujuan sudah tidak menerima booking baru.']);
            }

            if ($source->outgoingMovement()->exists()) {
                throw ValidationException::withMessages(['source_booking_id' => 'Booking sumber sudah pernah dipindahkan.']);
            }
            ...
            if ($duplicateDestinationDate) {
                throw ValidationException::withMessages(['destination_occurrence_id' => 'Anak sudah memiliki booking aktif lain pada tanggal tujuan.']);
            }

            if (! $capacityOverride && $destination->capacity !== null) {
                $activeCount = ChildSessionBooking::query()->active()->where('session_occurrence_id', $destination->id)->count();
